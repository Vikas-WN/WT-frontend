...
function StepCircle({ state, step }: { state: "completed" | "active" | "upcoming"; step: number }) {
  if (state === "completed") {
    return (
      <div className="relative z-10 flex size-7 shrink-0 items-center justify-center rounded-full bg-indigo-600 text-white ring-[3px] ring-indigo-100">
  ...
  return (
    <div className="relative z-10 flex size-7 shrink-0 items-center justify-center rounded-full border-2 border-slate-300 bg-white">
          ...
          <div key={n} className="flex flex-col items-center flex-1">
            <div className="flex items-center w-full">
              <div
                className={cn(
                  "flex-1 h-px",
                  idx === 0 ? "bg-transparent" : (n <= step ? "bg-indigo-400" : "bg-slate-200")
                )}
              />
              ...
              <div
                className={cn(
                  "flex-1 h-px",
                  isLast ? "bg-transparent" : (n < step ? "bg-indigo-400" : "bg-slate-200")
                )}
              />
            </div>
            <span
              className={cn(
                ...
                state === "active" && "text-indigo-600 font-semibold",
                state === "completed" && "text-indigo-600",
